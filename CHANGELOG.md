...
### Added
- **HOMECORE-ASSIST P2 — real LLM runners + pipeline fallback** (ADR-133). `homecore-assist` shipped P1 with only a `NoopRunner` stub; `AssistPipeline` never actually consulted it. Added two real `RufloRunner` implementations and wired `AssistPipeline::set_runner`: when the P1 regex recognizer finds no match, the pipeline now falls through to the configured runner for LLM-grade intent disambiguation, or returns the runner's free-form speech directly when it resolves no structured intent — only when neither the regex recognizer nor the runner can help does it fall back to "I'm not sure how to help with that." A regex match still always short-circuits the runner. **`HermesCliRunner`** is the one meant for actual use: it shells out to an already-installed [Hermes Agent](https://github.com/NousResearch/hermes-agent) CLI (`hermes --query "<utterance>" --quiet`, one process per query, plain-text stdout response) instead of inventing a new agent process — `spawn`/`shutdown` are no-ops since there's nothing persistent to manage. `SubprocessRufloRunner` also ships for the ADR-133 §1.1 long-lived `node ruflo-agent.js` model (newline-delimited JSON stdin/stdout, one request in flight at a time, Windows-safe explicit-`shutdown()` teardown per §Q3 option 2, `kill_on_drop` as a defence-in-depth safety net) for anyone building a bespoke MCP-over-stdio agent — no such script ships in this repo. 20 new tests (9 `SubprocessRufloRunner` + 5 `HermesCliRunner` tests against shell/Node.js test-double fixtures, 6 pipeline fallback/precedence tests with an in-process mock runner); `SemanticIntentRecognizer` (ruvector HNSW) and STT/TTS remain P2/P3 stubs per ADR-133 §Q4.
