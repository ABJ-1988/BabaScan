...
### Added
- **HOMECORE-ASSIST P2 — real ruflo subprocess runner + pipeline LLM fallback** (ADR-133). `homecore-assist` shipped P1 with only a `NoopRunner` stub; `AssistPipeline` never actually consulted it. Added `SubprocessRufloRunner`, a real `tokio::process::Child`-backed `RufloRunner` that manages a long-lived `node ruflo-agent.js` subprocess over a newline-delimited JSON stdin/stdout protocol (one request in flight at a time, per-request timeout), with the Windows-safe explicit-`shutdown()` teardown decided in ADR-133 §Q3 (option 2) plus `kill_on_drop` as a defence-in-depth safety net. `AssistPipeline::set_runner` wires it in: when the P1 regex recognizer finds no match, the pipeline now falls through to the configured runner for LLM-grade intent disambiguation, or returns the runner's free-form speech directly when it resolves no structured intent — only when neither the regex recognizer nor the runner can help does it fall back to "I'm not sure how to help with that." A regex match still always short-circuits the runner. 15 new tests (9 `SubprocessRufloRunner` tests against a Node.js test-double fixture, 6 pipeline fallback/precedence tests with an in-process mock runner); `SemanticIntentRecognizer` (ruvector HNSW) and STT/TTS remain P2/P3 stubs per ADR-133 §Q4.
